...
<div id="toastrS" class="toast d-none" role="alert" aria-live="polite" aria-atomic="true" data-delay="10000">
  <div class="alert alert-success" role="alert" aria-live="assertive" aria-atomic="true"></div>
</div>
<div id="toastrFa" class="toast d-none" role="alert" aria-live="polite" aria-atomic="true" data-delay="10000">
  <div class="alert alert-danger" role="alert" aria-live="assertive" aria-atomic="true"></div>
</div>
...
function CourseAdd(CourseName,CourseDes,CourseFee,CourseEnroll,CourseClass,CourseLink,CourseImg) {
    ...
    if(CourseName.length==0){
     $('#toastrFa').removeClass('d-none').toast('show').html("Course Name is Empty !");
    }
    else if(CourseDes.length==0){
     $('#toastrFa').removeClass('d-none').toast('show').html("Course Description is Empty !");
    }
    else if(CourseFee.length==0){
      $('#toastrFa').removeClass('d-none').toast('show').html("Course Fee is Empty !");
    }
    else if(CourseEnroll.length==0){
      $('#toastrFa').removeClass('d-none').toast('show').html("Course Enroll is Empty !");
    }
    else if(CourseClass.length==0){
      $('#toastrFa').removeClass('d-none').toast('show').html("Course Class is Empty !");
    }
    else if(CourseLink.length==0){
      $('#toastrFa').removeClass('d-none').toast('show').html("Course Link is Empty !");
    }
    else if(CourseImg.length==0){
      $('#toastrFa').removeClass('d-none').toast('show').html("Course Image is Empty !");
    }
        ...
        .then(function(response) {
            ...
            if(response.status==200){
              ...
              if (response.data == 1) {
                ...
                $('#toastrS').removeClass('d-none').toast('show').html("Add Success");
                getCoursesData();
            } else {
                $('#addCourseModal').modal('hide');
    ...
    axios.post('/CoursesDelete', {
        ...
        .then(function(response) {
            ...
            if (response.data == 1) {
                ...
                $('#deleteCourseModal').modal('hide');
                $('#toastrFa').removeClass('d-none').toast('show').html("Delete Fail");
				getCoursesData();
            }
...
function CourseUpdate(courseID,courseName,courseDes,courseFee,courseEnroll,courseClass,courseLink,courseImg) {
    ...
    if(courseName.length==0){
     ...
     t$('#toastrFa').removeClass('d-none').toast('show').html("Course Description is Empty !");
    }
    else if(courseFee.length==0){
      $('#toastrFa').removeClass('d-none').toast('show').html("Course Fee is Empty !");
    }
